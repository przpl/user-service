...
describe("CookieSessionManager", () => {
    ...
    describe("issueSession()", () => {
        ...
        it("should issue sessions with different ids", async () => {
            const ids: string[] = [];
            for (let i = 0; i < 5; i++) {
        ...
        it("should respect sessions limit", async () => {
            const config = mockConfig();
            const ids: string[] = [];
    ...
    describe("removeAllSessions()", () => {
        ...
        it("should remove not existing user sessions", async () => {
            const sid1 = await sut.issueSession("user1", "127.0.0.1", mockUserAgent());
    ...
    describe("removeSession()", () => {
        it("should remove not cached session", async () => {
            const sid1 = await sut.issueSession("user1", "127.0.0.1", mockUserAgent());
            const sid2 = await sut.issueSession("user1", "127.0.0.1", mockUserAgent());
        ...
        it("should remove cached session", async () => {
            const sid1 = await sut.issueSession("user1", "127.0.0.1", mockUserAgent());
            const sid2 = await sut.issueSession("user1", "127.0.0.1", mockUserAgent());
